 ...
 */
func main() {
	// Add() Done()：每次计数减一 Wait() 内部添加计数器，当计数大于0，阻塞，直到计数为 0。
	...
	go func(t *time.Ticker) {
		...
		for {
			<- t.C
			// 2006-01-02 15:04:05 必须使用这个时间，才可以格式化
			fmt.Println("get ticker1111", time.Now().Format("2006-01-02 15:04:05"))
		}
	}(ticker1)
	...
	go func(t *time.Timer) {
		...
		for {
			<- t.C
			fmt.Println("get timer2222", time.Now().Format("2006/01/02 15:04:05"))
